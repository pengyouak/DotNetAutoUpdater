...
{
    /// <summary>
    /// Provides a mechanism for storing AutoUpdater state between sessions based on storing data on
    ...
    public class RegistryPersistenceProvider : IPersistenceProvider
    {
        ...
        /// <summary>
        /// Initializes a new instance of the RegistryPersistenceProvider class indicating the path
        /// for the Windows registry key to use for storing the data.
        /// </summary>
            ...
            using (RegistryKey updateKey = Registry.CurrentUser.OpenSubKey(RegistryLocation))
            {
                ...
                if (remindLaterValue != null)
                {
                    return Convert.ToDateTime(remindLaterValue.ToString(),
                        CultureInfo.CreateSpecificCulture("zh-CN").DateTimeFormat);
                }
        ...
        public void SetRemindLater(DateTime? remindLaterAt)
        {
            using (RegistryKey autoUpdaterKey = Registry.CurrentUser.CreateSubKey(RegistryLocation))
            {
                autoUpdaterKey?.SetValue(RemindLaterValueName,
                    remindLaterAt != null
                        ? remindLaterAt.Value.ToString(CultureInfo.CreateSpecificCulture("zh-CN").DateTimeFormat)
                        : string.Empty);
            }
        }
    }
